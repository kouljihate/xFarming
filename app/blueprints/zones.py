...
def index():
    try:
        ...
        if isinstance(db, dict) and db.get('error'):
            ...
            return render_template('zones/index.html', zones=[], sectors=[], farms=[], search='')
        ...
        for farm in farms:
            ...
            for sector in farm.get('sectors', []):
                ...
                for zone in sector.get('zones', []):
                    ...
                    zone['farm_id'] = farm['_id']
                    zone['row_count'] = len(zone.get('rows', []))
                    zones.append(zone)

        return render_template('zones/index.html', zones=zones, sectors=sectors, farms=farms, search=search)
    ...
    try:
        ...
        try:
            ...
            zone_data = ZoneModel(
                ...
                location={
                    'area': {'value': _safe_float(request.form.get('area_value'), 0), 'unit': request.form.get('area_unit', 'ha')},
                    'row_spacing': {'value': _safe_float(request.form.get('row_spacing_value'), 0), 'unit': request.form.get('row_spacing_unit', 'feet')},
                    'tree_spacing': {'value': _safe_float(request.form.get('tree_spacing_value'), 0), 'unit': request.form.get('tree_spacing_unit', 'feet')},
                ...
                crop_info={
                    ...
                    'pollinators': [p.strip() for p in request.form.get('pollinators', '').split(',') if p.strip()]
                },
                soil_characteristics={
                    'type': request.form.get('soil_type', ''),
                    'ph': _safe_float(request.form.get('ph'), 0.0),
                    'organic_matter': request.form.get('organic_matter', ''),
                    'drainage': request.form.get('drainage', '')
                },
                statistics={
                    'total_rows': _safe_int(request.form.get('total_rows'), 0),
                    'total_trees': _safe_int(request.form.get('total_trees'), 0),
                    'trees_per_acre': _safe_int(request.form.get('trees_per_acre'), 0),
                    'active_trees': _safe_int(request.form.get('active_trees'), 0),
                    'dead_trees': _safe_int(request.form.get('dead_trees'), 0),
            ...
            if found:
                ...
                flash('Sector not found. Please select a valid sector.', 'danger')
            ...
            for sector in farm.get('sectors', []):
                ...
                for zone in sector.get('zones', []):
                    if str(zone.get('_id', '')) == zone_id:
                        ...
                        return render_template('zones/detail.html', zone=zone, sector=sector, farm=farm, rows=zone.get('rows', []))
                ...
                for zone in sector.get('zones', []):
                    if str(zone.get('_id', '')) == zone_id:
                        try:
                            ...
                            zone_data = ZoneModel(
                                zone_number=request.form.get('zone_number', zone.get('zone_number', '')),
                                name=request.form.get('name', ''),
                                description=request.form.get('description', ''),
                                location={
                                    'area': {'value': _safe_float(request.form.get('area_value')), 'unit': request.form.get('area_unit', 'ha')},
                                    'row_spacing': {'value': _safe_float(request.form.get('row_spacing_value')), 'unit': request.form.get('row_spacing_unit', 'feet')},
                                    'tree_spacing': {'value': _safe_float(request.form.get('tree_spacing_value')), 'unit': request.form.get('tree_spacing_unit', 'feet')},
                                    'orientation': request.form.get('orientation', '')
                                },
                                boundary=boundary,
                                crop_info={
                                ...
                                statistics={
                                    'total_rows': _safe_int(request.form.get('total_rows')),
                                    'total_trees': _safe_int(request.form.get('total_trees')),
                                    'trees_per_acre': _safe_int(request.form.get('trees_per_acre')),
                                ...
                                metadata={
                                    ...
                                    'status': request.form.get('status', zone.get('metadata', {}).get('status', 'active')),
                                    'notes': request.form.get('notes', zone.get('metadata', {}).get('notes', '')),
                                    'zone_manager': request.form.get('zone_manager', zone.get('metadata', {}).get('zone_manager', ''))
                                }
                            )
